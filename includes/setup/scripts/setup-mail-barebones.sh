...
ln -sf $CONFIGDIR/mail/postfix/postfix-main-barebones.cf /etc/postfix/main.cf
ln -sf $CONFIGDIR/mail/postfix/postfix-master-barebones.cf /etc/postfix/master.cf
ln -sf $CONFIGDIR/mail/dovecot/dovecot-99-barebones.conf /etc/dovecot/conf.d/99-local.conf
ln -sf $CONFIGDIR/system/rsyslogd/rsyslogd-10-postfix.conf /etc/rsyslog.d/10-postfix.conf
ln -sf $CONFIGDIR/system/supervisord/supervisord-barebones.conf /etc/supervisor/conf.d/supervisord.conf
...
EXPECTED_USER="$MAIL_USER@$IWB_DOMAIN"
...
if ! grep -q "^$EXPECTED_USER:" "$USERFILE" 2>/dev/null; then
  ...
  mkdir -p "/var/mail/vmail/$IWB_DOMAIN/$MAIL_USER/Maildir"/{cur,new,tmp}
  chown -R vmail:vmail "/var/mail/vmail/$IWB_DOMAIN"

  HASHED_PASS=$(doveadm pw -s SHA512-CRYPT -p "$MAIL_PASS")
  echo -e "$EXPECTED_USER:$HASHED_PASS:10000:10000::/var/mail/vmail/$IWB_DOMAIN/$MAIL_USER::" > "$USERFILE"
  chmod 600 "$USERFILE"
fi

# Ensure virtual_alias catch-all is in place
ALIASMAP=/etc/postfix/virtual_alias
EXPECTED_ALIAS="/.*/ ${MAIL_USER}@${IWB_DOMAIN}"
