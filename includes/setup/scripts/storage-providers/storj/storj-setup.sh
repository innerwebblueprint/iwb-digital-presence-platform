...
EOF
  ...
  if uplink access import default "$IWB_STORJ_GRANT" --force; then
  ...
  echo -e "$IWB_PREFIX Verifying Storj access for bucket: $IWB_STORJ_WPOPS_BUCKET"
  if ! uplink ls "sj://${IWB_STORJ_WPOPS_BUCKET}/"; then
    echo -e "$IWB_PREFIX $ERR_PREFIX Failed to verify Storj access. Check IWB_STORJ_GRANT and IWB_STORJ_WPOPS_BUCKET values."
    return 1
  else
    IWB_STORJSETUP=true
    #EXPORT IWB_STORJSETUP=true
    echo -e "$IWB_PREFIX Storj access verified successfully for bucket: $IWB_STORJ_WPOPS_BUCKET"
  fi
fi