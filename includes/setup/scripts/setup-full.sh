...
for TEMPLATE in "$SSL_TEMPLATE_DIR"/*ssl*.conf.template; do
  if [ -f "$TEMPLATE" ]; then
    ...
    -e "s|{{IWB_MEDIA_PROXY_BASE_URL}}|$IWB_MEDIA_PROXY_BASE_URL|g" \
    -e "s|{{IWB_MEDIA_PROXY_HOST}}|$IWB_MEDIA_PROXY_HOST|g" \
    "$TEMPLATE" > "$OUTPUT"
...
# Send startup notification email on every container start
log "Sending startup notification email to $IWB_MAIL_USER@$IWB_DOMAIN..."
source /var/setup/scripts/send-startup-email.sh &
